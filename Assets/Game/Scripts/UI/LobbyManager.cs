...
public class LobbyManager : MonoBehaviour
{
    ...
    public GameObject menuCampain;
    public List<GameObject> listMenus;
    public List<Button> listCampain;
    ...
    public void LoadStartUI()
    {
        ...
        startUI.GetComponentInChildren<Text>().text = player.toString();
    }

    public void LoadMenuCampain()
    {
        if (player == null)
        {
            return;
        ...
        CarregarPlayer(player.characterName);

        HabilitaCampain();

        AtivarMenu(menuCampain);
    }
    ...
    */
    ...
    public void HabilitaCampain()
    {
        for (int i = 0; i < listCampain.Count; i++)
        {
            if( i<= player.questComplete && i != listCampain.Count)
            {
                listCampain[i].interactable = true;
            }
            else
            {
                listCampain[i].interactable = false;
            }
        }
    }

    public void buttonOnClickCampain(int indiceCampain)
    {
        menuCampain.SetActive(false);
        PlayerPrefs.SetInt("indiceCampain", indiceCampain);
